...
import { X, Send, Bot, User, Copy, Trash2, Brain, Menu, Sparkles, Sun, Moon } from "lucide-react"
...
const OpenAIChatInterface = ({ isOpen, closeChat }) => {
  ...
  useEffect(() => {
    ...
    if (container && isOpen) {
      ...
      // Ensure the container has enough content to scroll
...
Your primary goal is to help visitors learn about George in a natural, conversational way.`,
      ...
      setTimeout(
        ...
        500 + Math.min(botResponse.length * 5, 2000),
      ) // Cap typing time at 2 seconds for long responses
    } catch (err) {
  ...
  const generateFallbackResponse = (userInput) => {
    ...
    if (normalizedInput.includes("who") && (normalizedInput.includes("you") || normalizedInput.includes("george"))) {
      ...
      normalizedInput.includes("tech stack")
    ) {
      setMessages((prev) => [
        ...prev,
        {
  ...
  const copyChat = () => {
    const chatText = messages.map((msg) => `${msg.sender === "bot" ? "AI Assistant" : "You"}: ${msg.text}`).join("\n\n")
            ...
            >
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className={`font-medium ${themeConfig[theme].text}`}>George&apos;s AI Assistant</h3>
                    <p className={`text-xs ${themeConfig[theme].textMuted}`}>Ask me anything about George</p>
                  </div>
                </div>
              ...
              <div
                ...
                className={`w-full flex-grow overflow-y-auto p-4 space-y-4 ${themeConfig[theme].primary} scrollbar-container`}
                style={{
              ...
              >
                    ...
                    >
                      <div className="flex items-start p-3 gap-2">
                        ...
                        <div className="space-y-2 flex-1 break-words">
                          <div
                            className={`${message.sender === "user" ? "text-white" : themeConfig[theme].text} text-sm sm:text-base`}
                          >
                            {message.sender === "bot" ? <SimpleMarkdownRenderer text={message.text} /> : message.text}
                          </div>
                          ...
                          <div
                            className={`text-xs ${message.sender === "user" ? "text-white/70" : themeConfig[theme].textMuted}`}
                          >
                ...
                {isThinking && (
                  ...
                  >
                    <div
                      className={`max-w-[85%] rounded-2xl overflow-hidden ${themeConfig[theme].accent} border ${themeConfig[theme].border}`}
                    >
              ...
              <div className={`p-4 border-t ${themeConfig[theme].border} backdrop-blur-sm mt-auto`}>
                <div className="relative">
                  <textarea
                    ...
                    placeholder="Ask about George's skills & experience..."
